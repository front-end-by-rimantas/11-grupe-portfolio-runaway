...
function renderClient ( testimonialsList ) {
    ...
    for ( let i=0; i<testimonialsList.length; i++ ) {
        const client = testimonials[Math.floor(Math.random() * testimonialsList.length)];
        listHTML +=  `<div class="testimonial ${i===client ? 'show' : ''}" data-index="${i}">
                            ...
                            <div class="description">${client.description}
                            </div> 
                    </div>`;
    }
